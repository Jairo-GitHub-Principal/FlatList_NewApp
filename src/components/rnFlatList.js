...
const newData = []
for(i= 0; i < 1000; i++ ){ // almentar o tamanho da listagem
newData.push({ 
            title: "Lorem ipsum", 
            text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut in congue risus, non viverra tellus. Nam faucibus ligula non metus ultrices mollis.",
...
const ItemComponent = ({itemRenderizado, onPress, backgroundColor, textColor,titleColor}) => (
  <Pressable onPress={onPress} style={[styles.itemEstilo, {backgroundColor}]}>
...
const RnFlatList = () => {
  ...
  const renderItem = ({item}) => {
    ...
    const backgroundColor = item.id === selectedId ? '#6e3b6e' : '#430000';
    const colorTitle = item.id === selectedId ? 'white' : 'red';
    const colorText = item.id ===selectedId ?'yellow':'white'
